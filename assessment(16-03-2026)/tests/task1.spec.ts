...
import flipkart from "../pom/pom.spec"
import path from "path";
import fs from "fs"
let fkart=JSON.parse(fs.readFileSync(path.join(__dirname,"../testdata/data.json")));
test("flipkart",async({page})=>{
    ...
    await page.screenshot({path:"screenshot/task1-1.png"})
    await flipkartsite.quantity(page,2,3);
    await page.screenshot({path:"screenshot/task1-2.png"})
    await flipkartsite.place_order(page);
    await page.screenshot({path:"screenshot/task1-3.png"})
})